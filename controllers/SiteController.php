...
//          ]);
    ...
    public function actionShowPatient()
    {
          ...
          SELECT  bed.ward
            ...
            (SELECT  ref
            FROM  nurse_event
            WHERE ward = ".$ward."  AND  patient_flag = '1'
        and date = (select max(date) from nurse_event where ward =".$ward." and patient_flag=1)
        and period = (select max(period) from nurse_event
        where ward =".$ward." and patient_flag=1 and
        date = (select max(date) from nurse_event where ward =".$ward." and patient_flag=1))
        ))  patient
            ON bed.bed_type  =  patient.bed_type
            AND bed.bed_no   =  patient.bed_no
            ORDER BY bed.ref,bed.bed_no
